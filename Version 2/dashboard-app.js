...
const state = { currentRange: "weekly", selectedMeetingId: null, editingScheduleId: null, editingHolidayId: null, holidays: [], meetings: [], todos: [], users: [], projects: [], schedules: [], authUser: null };
...
function updateDayOptions(range) { scheduleDayInput.innerHTML = boardLabels[range].map((day) => `<option value="${day}">${escapeHtml(day)}</option>`).join(""); }
function renderHealth() {
  const counts = { offtrack: 0, risk: 0, healthy: 0, done: 0 };
  state.projects.forEach((project) => { if (project.status === "Off track") counts.offtrack += 1; else if (project.status === "At risk") counts.risk += 1; else if (project.status === "Completed") counts.done += 1; else counts.healthy += 1; });
  healthBadges.innerHTML = [{ label: `${counts.offtrack} off track`, className: "offtrack" }, { label: `${counts.risk} at risk`, className: "risk" }, { label: `${counts.healthy} on track`, className: "healthy" }, { label: `${counts.done} completed`, className: "done" }].map((item) => `<span class="health-chip ${item.className}">${item.label}</span>`).join("");
}
...
function renderMeetings() {
  ...
  if (!selectedMeeting) { meetingTitle.textContent = "No meeting selected"; meetingMeta.textContent = "Choose any meeting from the list."; meetingNotes.value = ""; return; }
...
function resetScheduleForm() { state.editingScheduleId = null; scheduleSubmitBtn.textContent = "Add schedule"; scheduleForm.reset(); scheduleRangeInput.value = state.currentRange; updateDayOptions(scheduleRangeInput.value); }
function resetHolidayForm() { state.editingHolidayId = null; holidaySubmitBtn.textContent = "Save leave type"; holidayForm.reset(); }

logoutBtn.addEventListener("click", () => { sessionStorage.removeItem(SESSION_KEY); window.location.href = "/"; });
modeButtons.forEach((button) => button.addEventListener("click", () => { state.currentRange = button.dataset.range; modeButtons.forEach((pill) => pill.classList.toggle("active", pill === button)); scheduleRangeInput.value = state.currentRange; updateDayOptions(state.currentRange); renderDashboard(); renderScheduleList(); }));
scheduleRangeInput.addEventListener("change", () => updateDayOptions(scheduleRangeInput.value));

scheduleForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  const payload = { range: scheduleRangeInput.value, day: scheduleDayInput.value, title: scheduleTitleInput.value.trim(), note: scheduleNoteInput.value.trim(), color: scheduleColorInput.value };
  ...
  resetScheduleForm(); renderScheduleBoard(); renderScheduleList(); renderSidebar();
});
...
scheduleList.addEventListener("click", async (event) => {
  ...
  if (editId) {
    ...
    if (!schedule) return;
    state.editingScheduleId = schedule.id; scheduleSubmitBtn.textContent = "Update schedule"; scheduleRangeInput.value = schedule.range; updateDayOptions(schedule.range); scheduleDayInput.value = schedule.day; scheduleColorInput.value = schedule.color; scheduleTitleInput.value = schedule.title; scheduleNoteInput.value = schedule.note; return;
  }
  if (deleteId) {
    await apiRequest(`/api/schedules/${deleteId}`, { method: "DELETE" });
    state.schedules = state.schedules.filter((item) => item.id !== Number(deleteId));
    renderScheduleBoard(); renderScheduleList(); renderSidebar();
  }
});
holidayForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  const payload = { name: holidayNameInput.value.trim(), used: Number(holidayUsedInput.value), total: Number(holidayTotalInput.value) };
  ...
  resetHolidayForm(); renderHolidays();
});
...
holidayGrid.addEventListener("click", (event) => {
  ...
  if (!holiday) return;
  state.editingHolidayId = holiday.id; holidaySubmitBtn.textContent = "Update leave type"; holidayNameInput.value = holiday.name; holidayUsedInput.value = holiday.used; holidayTotalInput.value = holiday.total;
});
userForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  const name = userNameInput.value.trim(); const role = userRoleInput.value.trim(); const type = userTypeInput.value;
  if (!name || !role) return;
  const user = await apiRequest("/api/users", { method: "POST", body: JSON.stringify({ name, role, type }) });
  state.users.unshift(user); renderUsers(); renderProjects(); renderSidebar(); userForm.reset();
});
projectForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  const name = projectNameInput.value.trim(); const ownerId = Number(projectOwnerInput.value); const status = projectStatusInput.value;
  if (!name || !ownerId) return;
  const project = await apiRequest("/api/projects", { method: "POST", body: JSON.stringify({ name, ownerId, status }) });
  state.projects.unshift(project); renderProjects(); renderSidebar(); projectForm.reset(); projectOwnerInput.value = String(ownerId);
});
todoForm.addEventListener("submit", async (event) => {
  ...
  state.todos.unshift(todo); renderTodos(); todoForm.reset();
});
todoList.addEventListener("click", async (event) => {
  const deleteId = event.target.getAttribute("data-delete");
  if (!deleteId) return;
  await apiRequest(`/api/todos/${deleteId}`, { method: "DELETE" });
  state.todos = state.todos.filter((todo) => todo.id !== Number(deleteId)); renderTodos();
});
todoList.addEventListener("change", async (event) => {
  ...
  state.todos = state.todos.map((todo) => todo.id === updated.id ? updated : todo); renderTodos();
});
meetingList.addEventListener("click", (event) => {
  const meetingItem = event.target.closest(".meeting-item");
  if (!meetingItem) return;
  state.selectedMeetingId = Number(meetingItem.dataset.id); renderMeetings();
});
saveNotesBtn.addEventListener("click", async () => {
  ...
  state.meetings = state.meetings.map((meeting) => meeting.id === updated.id ? updated : meeting); renderMeetings(); saveNotesBtn.textContent = "Saved"; window.setTimeout(() => { saveNotesBtn.textContent = "Save notes"; }, 1200);
});
...
async function init() {
  ...
  state.holidays = data.holidays; state.meetings = data.meetings; state.todos = data.todos; state.users = data.users; state.projects = data.projects; state.schedules = data.schedules || []; state.selectedMeetingId = state.meetings[0]?.id || null;
  renderDashboard(); renderHolidays(); renderUsers(); renderProjects(); renderTodos(); renderMeetings(); renderSidebar(); renderScheduleBoard(); renderScheduleList();
  serverStatus.textContent = "Connected to local backend on localhost. Shared records are saving into db.json.";
}

init().catch((error) => { serverStatus.textContent = `Backend connection failed: ${error.message}`; });
